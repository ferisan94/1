...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedMeterPerSecond := meanSpeed(action, duration) * kmhInMsec
	heightInMeters := height / cmInM
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMeterPerSecond, 2)/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	calories := (speed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
	return calories
}
